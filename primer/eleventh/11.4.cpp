...
#include<utility>
using namespace std;
void addfamliy(map<string,vector<pair<string,string>>> & fam)
{
        ...
        if(n==1)  //添加家庭
    {
        ...
        cout << "Please enter ur family member last name(s) and birthday end to quit: \n";
        string Lnames,birthday;
        while(cin >> Lnames >> birthday)
        {   
            if(Lnames == "end")
            break;
            fam[Fname].push_back(make_pair(Lnames,birthday));
        }
...
int main()
{
    ...
    {   
        ...
        for(auto LnAndBirth:fa.second)
        {
            cout << LnAndBirth.first << " "<< LnAndBirth.second << endl;
        }
        cout << endl;
        cout << endl;
    }
